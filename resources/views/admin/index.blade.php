        ...
        <table class="min-w-full table-auto relative">
            ...
            <tbody class="text-gray-600 text-sm font-light">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100 relative">
                    <td class="py-3 px-6 text-left">{{ $loop->iteration + ($users->currentPage() - 1) * $users->perPage() }}</td>
                    ...
                    <td class="py-3 px-6 text-center">
                        <div class="relative inline-block text-left z-55">
                            <button onclick="toggleDropdown({{ $user->id }})" class="inline-flex items-center justify-center w-8 h-8 text-gray-600 hover:text-gray-800 focus:outline-none">
                            ...
                            <div id="dropdown-{{ $user->id }}"
                                ...
                                <a href="{{ route('admin.update', $user->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                    Edit
                                </a>
                                <form action="{{ route('admin.akunDestroy', $user->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus petugas ini?')">
                                    @csrf
                                    @method('DELETE')
